...
function FriendFilter () {
    ...
    const [update, setUpdate] = useState([...friends])

    const filterCity = (city) => {
        setUpdate(()=>{
        return friends.filter((friend) => friend.city === city)
        })
    }
    const resetFriends = () => {
        setUpdate([...friends])
  ...
  return (
    <>
      <button onClick={() => filterCity("seoul")}>서울</button>
      <button onClick={() => filterCity("busan")}>부산</button>
      <button onClick={() => filterCity("daegu")}>대구</button>
      <button onClick={() => resetFriends()}>전체</button>
      {update.map(({name, age, city}, i) => {
        return (
        <div key={i}>
...
// class FriendFilter extends Component {
//   state = {
//     friends: [
//       {name: 'victoria', age: 13, city: 'seoul'},
//       {name: 'sun', age: 34, city: 'busan'},
